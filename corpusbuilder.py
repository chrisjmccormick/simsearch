...
class CorpusBuilder(object):
    ...
    def buildCorpus2(self,
                    stop_words_file='stop_words.txt', 
                    titles_file='all-titles.txt',
                    docs_file='all-entries.txt',
                    tags_file='all-tags.txt'):
        
        print 'Reading stop word list...'
        # Read in all of the stop words (one per line) and store them as a set.
        # The call to f.read().splitlines() reads the lines without the newline
        # char.        
        with open(stop_words_file) as f:
        ...
        print 'Reading in all documents...'
        # Read in the documents (one per line)
        with open(docs_file) as f:
            documents = f.readlines()

        # Turn each 'document' into a list of words.
        # Remove the stop words and split by whitespace.
        # Note that my journals had already been tokenized by NLTK.
        print 'Tokenizing, removing stop words...'
        texts = [[word for word in document.lower().split() if word not in stoplist]
                 for document in documents]
        
        print 'Filtering out words that appear once...'
        ...
        # Print the top five most common words.
        # The list is sorted smallest to biggest, so...
        ...
        # Read in the journal tags (one entry per line), removing the newlines.
        with open('./all-tags.txt') as f:
